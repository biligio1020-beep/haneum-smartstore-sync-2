...
async function upload() {
  ...
  try {
    await client.access({
      host: must('CAFE24_FTP_HOST'),
      user: must('CAFE24_FTP_USER'),
      password: must('CAFE24_FTP_PASSWORD'),
      port: 21,
      secure: false
    });

    const remotePath =
      process.env.CAFE24_REMOTE_PRODUCTS_PATH ||
      '/newhome/data/products.js';

    await client.uploadFrom(OUTPUT, remotePath);
    console.log(`Cafe24 업로드 완료: ${remotePath}`);
  } finally {
    client.close();
  }
}
...
async function main() {
  ...
  await makeProductsJs(products);

  console.log('4/4 Cafe24 FTP 업로드');
  await upload();
